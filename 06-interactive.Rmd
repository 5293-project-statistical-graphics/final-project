...
We can choose the part to see the overview of time trends for these two packages. A decreasing trend for the commits of both packages over time. And an increasing trend for both packages in 2015.
```{r}
data <- tibble(date = rep(seq(2014, 2022 - 1/12, 1/12), 2), 
               repo = rep(c("ggplot2", "plotly"), each = 96)) %>%
  full_join(mutate_data(commit, "commit")) %>%
